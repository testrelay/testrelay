...
const AssignmentView = () => {
    ...
    const repoUrl = (url) => {
        if (url == null) {
            return (
                <span className="italic text-gray-400">repository not created, waiting for candidate to schedule</span>)
        }

        return (<a className="text-indigo-500" href={"https://" + url} rel="noreferrer"
                   target="_blank">{"https://" + url}</a>)
    }
    ...
    return (
        <div>
            ...
            <div>
                <div className="bg-white shadow-md rounded mt-10">
                    <div className>
...
const Reviewers = ({reviewers, assignment_id}) => {
    const [users, setUsers] = useState(reviewers);
    ...
    const formatted = users.map((r) => {
        ...
        if (r.user.github_username) {
            g = (<span className="ml-2">{r.user.github_username}</span>)
            colour = "gray"
        }
            ...
            <div key={r.user.id}>
                <div className="flex">
                    <div className="flex-grow">
                        <p className="mb-1 text-sm text-gray-600 font-bold">{r.user.email}</p>
                        {g}
                    </div>
                    <div>
                        <svg xmlns="http://www.w3.org/2000/svg" onClick={() => {
                            removeUser(r.user.id)
                        }} className="h-6 w-6 text-gray-600 hover:text-indigo-700 cursor-pointer" fill="none"
    ...
    return (<div className="border-t border-gray-200">
        ...
        <div className="bg-gray-50 py-4 px-8 rounded-b-lg">
            ...
            <p className="text-xs text-gray-600 italic leading-5">Don't see a member of your team here? <Link
                to="/users" className="text-indigo-600">Invite more users</Link> to your organisation to assign them as
                collaborators.</p>
        </div>
    </div>);
}
